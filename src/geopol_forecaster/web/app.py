...
"""FastAPI web application for Geopol Forecaster simulations.

Can run locally (`uvicorn geopol_forecaster.web.app:app`) or be mounted
...
def create_app(config_dir: str | Path | None = None):
    ...
    app = FastAPI(title="Geopol Forecaster", description="LLM Wargaming Simulator")
    ...
    async def index():
        ...
        if not index_path.exists():
            return HTMLResponse("<h1>Geopol Forecaster</h1><p>Static files not found.</p>")
    ...
    async def serve_report(filename: str):
        ...
        pdf_path = Path(".geopol_data/reports") / filename
...
def main():
    """Entry point for geopol-web CLI."""
    import uvicorn

    port = int(os.environ.get("GEOPOL_PORT", "8000"))
    print(f"\n  Geopol Forecaster Web GUI")
    print(f"  http://localhost:{port}")
    print()

    uvicorn.run(
        "geopol_forecaster.web.app:app",
        host="0.0.0.0",
        port=port,
        reload=False,
    )
